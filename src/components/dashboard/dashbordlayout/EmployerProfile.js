...
const EmployerProfile = () => {
    ...
    const getRequestData = () => {
        switch (activeStep) {
            ...
            case 2: return { filter_type: 'email', email: formData.email, email_type: formData.emailType };
            case 3: return { filter_type: 'location', state_id: formData.workState, district_id: formData.workDistrict, country_id: formData.workCountry, work_location_name: formData.businessLocation, total_employees: formData.totalEmployees };
        ...
        switch (activeStep) {
            ...
            case 2: return (
                <Box>
                    ...
                    <Dropdown label="Email Type" name="emailType" value={formData.emailType} onChange={handleChange} options={['HR', 'Communication', 'Support', 'Finance', 'Other']} />
                </Box>
            );
            case 3: return (
                <Box>
                    <FormField label="Total Employees" name="totalEmployees" value={formData.totalEmployees} onChange={handleChange} type="number" />
                    <Dropdown label="Country" name="workCountry" value={formData.workCountry} onChange={handleChange} options={countries.map(c => ({ id: c.id, name: c.country }))} />
                    <Dropdown label="State" name="workState" value={formData.workState} onChange={handleChange} options={states.map(s => ({ id: s.id, name: s.state }))} />
                    <Dropdown label="District" name="workDistrict" value={formData.workDistrict} onChange={handleChange} options={districts.map(d => ({ id: d.id, name: d.district }))} />
                </Box>
